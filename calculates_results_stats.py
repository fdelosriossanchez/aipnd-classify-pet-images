...
def calculates_results_stats(results_dic):
    ...
    results_stats_dic["n_dogs_img"] = len(
        [x for x in results_dic.values() if x[3] == 1]
    )
    results_stats_dic["n_notdogs_img"] = len(
        [x for x in results_dic.values() if x[3] == 0]
    )
    results_stats_dic["n_match"] = len([x for x in results_dic.values() if x[2] == 1])
    results_stats_dic["n_correct_dogs"] = len(
        [x for x in results_dic.values() if x[3] == x[4] == 1]
    )
    results_stats_dic["n_correct_notdogs"] = len(
        [x for x in results_dic.values() if x[3] == x[4] == 0]
    )
    results_stats_dic["n_correct_breed"] = len(
        [x for x in results_dic.values() if x[2] == x[3] == 1]
    )
